...
while True:
    ...
    try:
        # Extract faces and their embeddings using DeepFace
        faces = DeepFace.extract_faces(imgRGB, enforce_detection=False)
        encodeCurFrame = [DeepFace.represent(face["face"], model_name='VGG-Face', enforce_detection=False)[0]["embedding"] for face in faces]
        faceCurFrame = [face["facial_area"] for face in faces]
    except ValueError as e:
        print(f"Error extracting faces or embeddings: {e}")
        faces = []
        encodeCurFrame = []
        faceCurFrame = []
